...
public class WorldController extends GraphicsProgram {
	...
	public void addLabels() {
		...
		double change = ((theWorld.getCurrentTotalGDPDouble() - theWorld.getPreviousTotalGDPDouble())
				/ theWorld.getPreviousTotalGDPDouble()) * 100;
	...
	public void colorSquares() { // Take in the locations from the companies and fill the world with the
									// appropriate color
		for (int i = 0; i < theWorld.getoilCompanyList().size(); i++) {
			...
			for (int j = 0; j < points.size(); j++) {
				...
				GRect r = new GRect(l.getX(), l.getY(), 15, 15); // Each square corresponds to $10000000 in each
																	// company's budget
	...
	public void acceptUserInput(int x, int y) {
		Company company = theWorld.oilCompanyList.get(x / 45);
		company.setTypeOfCompany(dialog.getCompanyType());
		company.setMyBudget(dialog.getBudget());
	}
}
